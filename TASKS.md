...
# Phase 7 — Management & Integrity

**Status:** Pending

UAT-driven additions. Addresses import of existing projects, phase/task CRUD, file repair, state sync, and a live dependency engine. The dependency engine (deps.py) is foundational and must be built first — all other Phase 7 write commands depend on it as a pre-write gate.
...
**Reviewer:** Skip
**Key constraints:**
- deps.py must have zero imports from any command module — it takes list[Phase] only
- detect_cycles() must handle both direct cycles (A → B → A) and indirect cycles (A → B → C → A)
- check_deps() must be callable as a pre-write gate — it returns list[str] errors, never raises
**Done when:**
...
- test_deps_cycle_detection passes: A→B→A cycle detected; path contains [A, B, A] or equivalent; clean graph returns []
...
- All functions operate on raw string content only — never re-serialize from the data model
...
**Done when:**
- insert_phase_block → re-parse → new phase at correct position; all other phase content byte-identical
- remove_phase_block → re-parse → phase gone; all other phases byte-identical
- insert_task_block → re-parse → task at correct position; dep graph block is still last in its phase
- remove_task_block → re-parse → task gone; all other task raw_blocks byte-identical
- update_phase_structure_table → re-parse → PhaseOverviewRows match input; all task blocks unchanged
- update_task_field for a multi-line What field → re-parse → task.what equals new value; all other task fields unchanged
- update_task_field adding Key constraints to a task that had none → re-parse → key_constraints populated; field appears before Done when
...
**Reviewer:** Skip
**Key constraints:**
- reorder_phase_blocks is the highest-risk function in Phase 7 — verify with a byte diff that content within each block is identical before and after reordering
...
- reorder_task_blocks → re-parse → tasks in new order within the phase; dep graph still last; all task raw_blocks byte-identical
- reorder_phase_blocks with a missing ID → ValueError
- reorder_task_blocks with dep graph ID in the list → dep graph still ends up last
...
### P7-T05 · commands/task.py — task CRUD commands

**Status:** Pending
**Complexity:** medium
**What:** Implement tsm/commands/task.py with four functions: task_add(ctx, phase_id, title, after_task_id, interactive) -> list[PendingWrite]; task_edit(ctx, task_id, field, value, interactive) -> list[PendingWrite]; task_move(ctx, task_id, phase_id, after_task_id) -> list[PendingWrite]; task_remove(ctx, task_id, force) -> list[PendingWrite]. task_add: auto-generates ID per §14 algorithm; if interactive=True, launches TaskFormOverlay (P7-T06). task_edit: non-interactive uses update_task_status() for the status field, update_task_field() (§9.4a) for all other fields; editing hard_deps runs dep gate on proposed state; interactive launches TaskFormOverlay pre-populated then applies update_task_field() for each changed field. task_move: if task is active/up-next in SESSIONSTATE.md, includes SESSIONSTATE.md PendingWrite in result. task_remove: applies transformation to proposed in-memory state, runs dep gate, blocks without --force. All functions call check_deps() on proposed state pre-write. Add HELP_TEXT. Wire into __main__.py. Implements §15.3.
**Prerequisite:** P7-T03b complete.
**Hard deps:** P7-T03b
**Files:** tsm/commands/task.py, tests/commands/test_task.py
**Reviewer:** Skip
**Key constraints:**
- task_edit must use update_task_status() for the status field and update_task_field() (§9.4a) for all other fields — never use update_task_field() for status
...
**Status:** Pending
**Complexity:** medium
**What:** Implement tsm/commands/sync.py with sync(ctx: LoadedProject, from_task_id: str) -> list[PendingWrite]. Algorithm per §15.5 v1.6: (1) Locate from_task_id — exit 1 with "Unknown task ID: <id>" if not found. (2) Apply in-memory: all tasks before from_task_id in its phase → COMPLETE; from_task_id → ACTIVE; all tasks after → PENDING; all phases before current → COMPLETE (all tasks within, overriding any current status); all phases after → PENDING. (3) Detect parallel dep chains — emit the §15.5 warning prompt if found. (4) Identify tasks newly set to COMPLETE in this operation (were not already COMPLETE before sync) to avoid re-appending. (5) Build PendingWrites: TASKS.md via update_task_status/update_phase_status; SESSIONSTATE.md via session_writer (clear completed table); TASKS-COMPLETED.md via completed_writer for newly-completed tasks only with blank commit. Add HELP_TEXT. Implements §15.5.
**Prerequisite:** P7-T01 complete.
**Hard deps:** P7-T01, P3-T03, P3-T04, P3-T05
**Files:** tsm/commands/sync.py, tests/commands/test_sync.py
**Reviewer:** Skip
**Key constraints:**
- Use update_task_status() and update_phase_status() for all TASKS.md writes — never re-serialize TASKS.md from data model
- Only append to TASKS-COMPLETED.md tasks that were not already COMPLETE before sync — never re-append
- SESSIONSTATE.md completed table is cleared on sync — it reflects per-session history, not bulk reconciliation
**Done when:**
...
### P7-T09 · commands/import_cmd.py — LLM-assisted normalization

**Status:** Pending
**Complexity:** high
**What:** Implement tsm/commands/import_cmd.py (named to avoid shadowing Python's import keyword) with import_files(ctx: LoadedProject, tasks_path: Path | None, session_path: Path | None) -> list[PendingWrite]. Read API key from .tsm/config.toml using the tomllib stdlib module (Python 3.11+); exit 1 with setup instructions from §15.1.1 if missing. Use the exact system prompt template from §15.1.2 as a module-level string constant. Send raw file content to API; parse and validate JSON response per §15.1.2 validation rules; fill missing fields with safe defaults. Serialize normalized output to canonical tsm format using existing writer functions (insert_phase_block, insert_task_block) — not ad-hoc string building. On API error, exit 1 and save raw response to .tsm/import-debug.txt. Confirm summary lists every field that was defaulted. Add HELP_TEXT. Implements §15.1.
...
**Reviewer:** Skip
**Key constraints:**
- API key must be read from .tsm/config.toml via tomllib only — never from environment variables or sys.argv
- Network calls are isolated to this module only — no other module may import anthropic
- Use the exact system prompt template from §15.1.2 as a module-level constant — do not paraphrase or shorten it
...
**Done when:**
- test_import_normalizes_tasks passes (mock API response returning valid JSON per §15.1.2 schema)
- test_import_fills_missing_fields_with_defaults passes
- test_import_no_api_key passes (no config.toml → exit 1, setup instructions printed)
- test_import_api_error passes (mocked error → exit 1, .tsm/import-debug.txt written)
...
```
P7-T01 ──► P7-T02
P7-T01 ──► P7-T03a ──► P7-T03b ──► P7-T04
                   └──► P7-T05 (P7-T03b also required)
                   └──► P7-T09
P6-T05 ──► P7-T06
P7-T01 ──► P7-T07
P7-T01 ──► P7-T08
